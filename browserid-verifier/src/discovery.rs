...
use browserid_dnssec::DnsFetcher;
use crate::error::VerifierError;
use crate::verify::HttpFetcher;

/// Wrapper to run sync HTTP fetch in a blocking task
async fn fetch_blocking(host: &str) -> Result<SupportDocument, VerifierError> {
    ...
    .await
    .map_err(|e| VerifierError::Discovery(format!("Blocking task failed: {}", e)))?;

    result.map_err(|e| VerifierError::Discovery(format!("HTTP fetch failed: {}", e)))
}
...
async fn fetch_blocking_http(host: &str) -> Result<SupportDocument, VerifierError> {
    let host = host.to_string();
...
impl FallbackFetcher {
    ...
    pub async fn discover(&self, domain: &str) -> Result<FallbackResult, VerifierError> {
        ...
        match dns_result.dnssec_status {
            ...
            DnssecStatus::Bogus => {
                ...
                Err(VerifierError::DnssecValidationFailed {
    ...
    async fn fallback_to_broker(&self) -> Result<FallbackResult, VerifierError> {
        ...
        match dns_result.dnssec_status {
            DnssecStatus::Secure => match dns_result.record {
                ...
                None => Err(VerifierError::Discovery(format!(
            ...
            DnssecStatus::Insecure => {
                if self.dev_local_broker {
                    ...
                    Err(VerifierError::Discovery(format!(
                        "trusted broker '{}' discovery is not DNSSEC-authenticated",
                        broker
                    )))
                }
            }
            DnssecStatus::Bogus => Err(VerifierError::DnssecValidationFailed { domain: broker }),
        }
    }
}
...
) -> Result<SupportDocument, VerifierError> {
