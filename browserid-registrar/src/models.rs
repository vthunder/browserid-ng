...
pub struct WarrantGrantItem {
    pub audience: String,
    /// Scope ENTRIES (spec §5): a bare string or `{ scope, …parameters }`.
    /// Preserved verbatim into the signed warrant; an entry's identity for
    /// fingerprints and status subjects is its scope string.
    #[serde(default)]
