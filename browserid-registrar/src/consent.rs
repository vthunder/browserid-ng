...
/// Per-delegator cap on simultaneously pending external requests (§6.6
/// anti-spam, with redirect-tied delivery doing the real work)
const MAX_PENDING_EXTERNAL: usize = 5;
...
/// POST /warrant/request — a consent request for one or more warrants, in
/// one of two shapes distinguished by the bundle's part count:
///
/// - **Own-agent** (`U_cert~P_cert~R`, 3 parts): an agent holding a
///   registered provisioning credential asks its delegator. Verified
///   exactly like `/provision/endorse`: the signed bundle against the
///   registry; the registry is the gate.
/// - **External** (`agent_cert~R`, 2 parts, §6.6): a service agent
///   certified by a *foreign* IdP asks a delegator rooted here. No registry
///   gate exists, so the gates are: `R` signed by the certified agent key
///   (issuer key via discovery), redirect-tied delivery (the row is only
///   reachable through its `/consent/<code>` link — unsolicited requests
///   are never surfaced and just expire), and a per-delegator cap on
///   pending external rows.
pub async fn request(
    ...
    if req.request_bundle.matches('~').count() == 1 {
        return request_external(state, &req.request_bundle).await;
    }
...
/// The external branch of `/warrant/request` (§6.6): verify the
/// `agent_cert~R` bundle against the foreign issuer's discovered key, route
/// the delegator hint to a local account, rate-limit, and park the request
/// for its redirect-tied `/consent/<code>` pickup.
async fn request_external(
    state: Arc<RegistrarState>,
    bundle: &str,
) -> Result<Json<WarrantRequestResponse>, RegistrarError> {
    let resolver = state.issuer_resolver.as_ref().ok_or_else(|| {
        RegistrarError::PolicyRefused(
            "external warrant requests are not accepted at this registrar".into(),
        )
    })?;

    let bundle = browserid_core::ExternalWarrantRequest::parse(bundle)
        .map_err(|e| RegistrarError::InvalidProvisioningRequest(e.to_string()))?;

    // Gate on the request's own (signed-but-not-yet-verified) claims BEFORE
    // spending an outbound discovery round-trip or allocating any state: a
    // request that doesn't target this registrar, names no local delegator,
    // or whose delegator is already at the pending cap costs nothing. This
    // keeps the anonymous endpoint from being a discovery-amplification or
    // pre-consent status-allocation lever against arbitrary domains. The
    // signature over these same claims is checked below, so this gate can
    // only ever *narrow* what proceeds.
    let claims = bundle.request().claims();
    if claims.domain != state.domain {
        return Err(RegistrarError::InvalidProvisioningRequest(
            "request domain does not target this registrar".into(),
        ));
    ...
    let delegator = claims
        ...
        .filter(|d| !d.is_empty())
        .ok_or_else(|| {
            RegistrarError::InvalidProvisioningRequest("missing delegator".into())
        })?;
    let user_id = state
        .host
        ...
        .ok_or_else(|| {
            RegistrarError::PolicyRefused(
                "the delegator is not an identity rooted at this registrar".into(),
            )
        })?;
    // Anti-spam backstop: unsolicited rows are unreachable anyway
    // (redirect-tied), so this cap just bounds what a misbehaving requester
    // can park — and, checked here, what it can make us discover/allocate —
    // per delegator.
    let pending_external = state
        .store
        .list_pending_warrant_requests(user_id)?
        .iter()
        .filter(|r| r.external)
        .count();
    if pending_external >= MAX_PENDING_EXTERNAL {
        return Err(RegistrarError::PolicyRefused(
            "too many pending external requests for this delegator".into(),
        ));
    }

    // Now spend the discovery round-trip and verify the signatures.
    let issuer_key = resolver
        .resolve_issuer_key(bundle.agent_cert().issuer())
        .await?;
    let verified = bundle
        .verify(&issuer_key)
        .map_err(|e| RegistrarError::InvalidProvisioningRequest(e.to_string()))?;
    // The verified (signed) claims must still target this registrar and name
    // the delegator we gated on — else a valid signature over *different*
    // claims than the pre-gate ones slipped through.
    if verified.request.domain != state.domain
        || !verified.delegator.eq_ignore_ascii_case(delegator)
    {
        return Err(RegistrarError::InvalidProvisioningRequest(
            "verified request claims do not match the request".into(),
        ));
    }
    let grants = verified.request.warrant_grants.clone().unwrap_or_default();
    validate_grants(&grants)?;
    ...
    create_pending_request(
        ...
        user_id,
        &verified.delegator,
        &verified.agent_email,
        // Display label: the foreign IdP that certified the agent.
        &verified.agent_issuer,
        grants,
        true,
    )
    .map(Json)
...
/// Shared grant validation for both request shapes. The count/wildcard
/// rules mirror the signer-side checks in `browserid_core`; the length and
/// character rules exist because the audience is rendered verbatim on the
/// consent page, where padding or invisible characters could disguise what
/// is being approved.
fn validate_grants(grants: &[browserid_core::WarrantGrant]) -> Result<(), RegistrarError> {
...
/// Allocate per-grant status indexes (egr7 — the consent page embeds them,
/// so each single warrant is revocable on its own), insert the pending row,
/// and shape the RFC 8628 response.
fn create_pending_request(
    ...
    agent_email: &str,
    label: &str,
    grants: Vec<browserid_core::WarrantGrant>,
    external: bool,
) -> Result<WarrantRequestResponse, RegistrarError> {
    let code = new_code();
    let now = Utc::now();
    ...
    tracing::info!(delegator = %delegator_email, external, "created warrant consent request");
    Ok(WarrantRequestResponse {
        success: true,
