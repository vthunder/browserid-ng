...
updated_at: 2026-09-11T16:50:18Z
...
- [x] (2026-09-11) derived-identity 'signs in via' labels: the device flow now carries parent_email in the device-authorize handback (mingo-idp /device_cert → dialog storeDevicePair → /wsapi/set_parent). Was: the device flow doesn't call /wsapi/set_parent (no subordinate_to channel in the device-authorize handshake) so new handle identities lack the parent link in the broker chooser
