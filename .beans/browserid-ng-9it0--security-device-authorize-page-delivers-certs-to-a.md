...
Fixed in static/common/js/idp-device-authorize.js: return_url is honored only when it parses and is same-origin with the validated return_origin; otherwise it is dropped at parse time, so both fail() and deliver() fall back to the (origin-checked) postMessage lane. All current legit consumers (wallet primaryHop, dialog resume lanes) already pass same-origin pairs, so no behavior change for them. Regression spec added: e2e-tests/tests/device-authorize-return-url.spec.ts (3 tests, backend stubbed via page.route). Full Playwright suite green (109 passed / 15 skipped). Residual risk (attacker controls BOTH params + session-skip silent issuance) filed as bean mxcn.
