...
- [ ] All-zero placeholder pubkey in SupportDocument::delegate()/disabled() (discovery.rs:62,73) — make it Option or a distinct variant.
...
- [ ] auth_with_assertion cold-start bug: uses get_fallback_fetcher() (non-initializing) so primary login can fail with 'DNS discovery not configured' if /verify wasn't hit yet (primary.rs:44-46). Use the initializing accessor.
