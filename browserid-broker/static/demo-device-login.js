// Device-cert model demo — a relying party logging you in via the device-cert flow.
//
// In-browser, against this broker (acting as IdP + hosted broker + verifier):
//   1. /device/issue     → an IdP-signed device cert (authentication) for a device key
//   2. sign an access request with the device key
//   3. /access/mint      → a fresh-key access cert (the device cert is the credential)
//   4. /warrant/issue    → a warrant (identity,subject→audience) + its config cert
//   5. sign an assertion with the access key; present access_cert~assertion~warrant~config_cert
//   6. /verify-access    → the RP verifies the bundle
//
// See docs/design/browserid-end-to-end-flow.md.

const enc = new TextEncoder();
const b64urlJson = (obj) =>
  btoa(String.fromCharCode(...enc.encode(JSON.stringify(obj))))
    .replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
const nowS = () => Math.floor(Date.now() / 1000);
const HDR = b64urlJson({ alg: 'EdDSA', typ: 'JWT' });
const genKeypair = () => new Promise((res, rej) =>
  window.jwcrypto.generateKeypair({ algorithm: 'Ed25519' }, (e, kp) => (e ? rej(e) : res(kp))));
const rawSign = (key, msg) => new Promise((res, rej) => key.sign(msg, (e, s) => (e ? rej(e) : res(s))));
async function signJws(kp, payloadB64) {
  const sk = window.jwcrypto.loadSecretKeyFromObject({ algorithm: 'Ed25519', d: kp.secretKey._jwk.d, x: kp.publicKey._jwk.x });
  const sig = await rawSign(sk, `${HDR}.${payloadB64}`);
  return `${HDR}.${payloadB64}.${sig}`;
}
async function postJSON(path, body) {
  const r = await fetch(path, { method: 'POST', headers: { 'content-type': 'application/json' }, credentials: 'same-origin', body: JSON.stringify(body) });
  return { ok: r.ok, status: r.status, body: await r.json().catch(() => ({})) };
}
const getJSON = (p) => fetch(p, { credentials: 'same-origin' }).then((r) => r.json());
const decodePayload = (jws) => JSON.parse(atob(jws.split('.')[1].replace(/-/g, '+').replace(/_/g, '/')));

const steps = document.getElementById('steps');
function step(label) {
  const li = document.createElement('li');
  li.className = 'pending';
  li.innerHTML = '<span class="mark">…</span> <span class="label"></span><div class="detail"></div>';
  li.querySelector('.label').textContent = label;
  steps.appendChild(li);
  return {
    ok: (d) => { li.className = 'ok'; li.querySelector('.mark').textContent = '✓'; if (d) li.querySelector('.detail').textContent = d; },
    fail: (d) => { li.className = 'err'; li.querySelector('.mark').textContent = '✗'; if (d) li.querySelector('.detail').textContent = d; },
  };
}

async function run() {
  document.getElementById('run').disabled = true;
  steps.innerHTML = '';
  document.getElementById('result').hidden = true;
  const email = document.getElementById('email').value.trim().toLowerCase();
  if (!email) { alert('Enter the email you are signed in with.'); document.getElementById('run').disabled = false; return; }

  try {
    let s = step('Check broker session');
    const ctx = await getJSON('/wsapi/session_context');
    if (!ctx.authenticated) { s.fail('Not signed in. Open /account, sign in, then retry.'); document.getElementById('run').disabled = false; return; }
    const csrf = ctx.csrf_token, idpDomain = ctx.domain, audience = location.origin;
    s.ok(`signed in · IdP ${idpDomain}`);

    // 1. Device cert (authentication).
    s = step('Issue a device cert (authentication) for this device');
    const device = await genKeypair();
    let r = await postJSON('/device/issue', { csrf, email, device_pubkey: device.publicKey._jwk.x });
    if (!r.ok || !r.body.device_cert) { s.fail(`device/issue: ${r.body.reason || r.status} (is "${email}" a verified email on your account?)`); document.getElementById('run').disabled = false; return; }
    const deviceCert = r.body.device_cert;
    s.ok('device cert issued (holds purpose=authentication, subject=user)');

    // 2 + 3. Access request signed by the device key → access cert.
    s = step('Mint a fresh-key access cert via /access/mint');
    const access = await genKeypair();
    const areqPayload = b64urlJson({
      typ: 'browserid-access-request-v1', iat: nowS(), exp: nowS() + 600,
      domain: idpDomain, identity: email, subject: 'user',
      'access-key': { algorithm: 'Ed25519', publicKey: access.publicKey._jwk.x },
    });
    const accessRequest = await signJws(device, areqPayload);
    r = await postJSON('/access/mint', { device_cert: deviceCert, access_request: accessRequest });
    if (!r.ok || !r.body.access_cert) { s.fail(`access/mint: ${r.body.reason || r.status}`); document.getElementById('run').disabled = false; return; }
    const accessCert = r.body.access_cert;
    s.ok('access cert minted (fresh key; device cert stays private)');

    // 4. Warrant + config cert.
    s = step('Get a warrant (identity,subject→audience) + its config cert');
    r = await postJSON('/warrant/issue', { csrf, email, audience });
    if (!r.ok || !r.body.warrant) { s.fail(`warrant/issue: ${r.body.reason || r.status}`); document.getElementById('run').disabled = false; return; }
    const warrant = r.body.warrant, configCert = r.body.config_cert;
    s.ok('warrant issued + signed by the broker config cert');

    // 5. Assertion + present the bundle.
    s = step('Sign an assertion and present the bundle to the RP verifier');
    const assertion = await signJws(access, b64urlJson({ exp: nowS() + 300, aud: audience }));
    const presentation = `${accessCert}~${assertion}~${warrant}~${configCert}`;
    r = await postJSON('/verify-access', { presentation, audience });
    if (r.body.status !== 'okay') { s.fail(`verify-access: ${r.body.reason || r.status}`); document.getElementById('run').disabled = false; return; }
    s.ok(`/verify-access OK → ${r.body.email} · subject=${r.body.subject} · scopes=${(r.body.scopes || []).join(',')}`);

    // Success panel.
    const ac = decodePayload(accessCert), wc = decodePayload(warrant), cc = decodePayload(configCert);
    document.getElementById('r-email').textContent = r.body.email;
    document.getElementById('r-access').textContent = `identity=${ac.identity} subject=${ac.subject} iss=${ac.iss} (certifies a fresh key, exp ${new Date(ac.exp * 1000).toISOString()})`;
    document.getElementById('r-warrant').textContent = `${wc.identifier}, subject ${wc.subject} → ${wc.audience} · scopes ${(wc.scopes || []).join(',')}`;
    document.getElementById('r-config').textContent = `purpose=${cc.purpose} subject=${cc.subject} identities=${(cc.identities || []).join(',')} iss=${cc.iss}`;
    document.getElementById('result').hidden = false;
  } catch (e) {
    step('Unexpected error').fail(String(e && e.message ? e.message : e));
  }
  document.getElementById('run').disabled = false;
}

document.getElementById('run').addEventListener('click', run);
getJSON('/wsapi/session_context').then((ctx) => {
  const b = document.getElementById('signin-note');
  if (ctx && ctx.authenticated) b.textContent = `Signed in to ${ctx.domain}. Enter the email you signed in with and run the flow.`;
  else b.innerHTML = 'You are not signed in. <a href="/account">Sign in at /account</a> first, then reload.';
}).catch(() => {});
