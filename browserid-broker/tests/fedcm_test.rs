...
//! Provider for fallback identities. Proves the token the (server-side) FedCM
...
async fn fedcm_mints_verifiable_assertion() {
    ...
    let ok = verify_assertion_with_dns(&token, AUDIENCE, &disc, &[BROKER.to_string()]).await;
    ...
    let wrong_aud =
        verify_assertion_with_dns(&token, "https://evil.example.com", &disc, &[BROKER.to_string()])
            .await;
    assert_eq!(wrong_aud.status, "failure");

    // Fallback gate: an RP that does NOT accept this broker as a fallback rejects it.
    let not_accepted =
        verify_assertion_with_dns(&token, AUDIENCE, &disc, &["other.example".to_string()]).await;
