    ...
    E: EmailSender + 'static,
{
    Router::new()
        ...
        // Passwordless external login + federated @mingo.place provisioning (T1)
        .route("/wsapi/stage_login", post(account::stage_login))
        .route("/wsapi/complete_login", post(account::complete_login))
        .route("/wsapi/provision_mingo", post(account::provision_mingo))
