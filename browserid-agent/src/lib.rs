...
// ===========================================================================
// Device-cert model (DC Phase 7) — the headless agent path.
//
// Added ADDITIVELY alongside the legacy delegation SDK above. An agent here is
// a *device* holding an IdP-issued AGENT DEVICE CERT (purpose=authentication,
// subject=agent). It signs an access request with its device key, POSTs the
// IdP's `/access/mint`, and receives a short-lived access cert for a fresh key.
// To present at an RP it assembles `access_cert~assertion~warrant~config_cert`,
// using a held user-config-cert-signed warrant (+ the config cert) per audience.
// ===========================================================================

use browserid_core::device::{
    AccessCert, AccessPresentation, AccessRequest, DeviceCert, Subject,
