...
const { generateKey, jws, proof, kidOf, decodeJws, sha256b64u } = require('./crypto.js');
...
// Registry request proof (registry-api-v1 §4.4): its OWN typ (domain
// separation), kid naming the key, htm/htu/iat/jti, bh binding a body.
{
  const kp = await generateKey();
  const p = await proof(kp.privJwk, 'POST', 'https://registry.example/api/v1/login', { body: '{"a":1}', x: kp.x });
  const header = JSON.parse(b64ud(p.split('.')[0]).toString());
  assert.deepEqual(header, { alg: 'EdDSA', typ: 'browserid-registry-proof-v1', kid: kidOf(kp.x) });
  const claims = decodeJws(p);
  assert.equal(claims.htm, 'POST');
  assert.equal(claims.htu, 'https://registry.example/api/v1/login');
  assert.equal(claims.bh, sha256b64u('{"a":1}'));
  assert.ok(claims.jti.length >= 16 && Math.abs(claims.iat - Date.now() / 1000) < 5);
  assert.equal(await verifyJws(p, kp.x), true);
  // A GET / possession proof carries no bh; jti is fresh per proof unless pinned.
  const p2 = await proof(kp.privJwk, 'GET', 'https://registry.example/api/v1/requests', { x: kp.x });
  assert.equal(decodeJws(p2).bh, undefined);
  assert.notEqual(decodeJws(p2).jti, claims.jti);
  const p3 = await proof(kp.privJwk, 'GET', 'https://registry.example/api/v1/requests', { x: kp.x, jti: 'shared' });
  assert.equal(decodeJws(p3).jti, 'shared');
  console.log('✓ registry proof: typ + kid pinned, bh binds the body, fresh jti');
}
